...
//Bipartite_graph
bool isBipartite_(int src, vector<int>& visited)
{
    ...
    while(que.size() != 0)
    {
        ...
        while(size--)
        {
            ...
            if(visited[rvtx.first] != -1) // Cycle -> already visited
            {
                if(visited[rvtx.first] != rvtx.second)
                    return 0;
            }
...
void isBipartite()
{
    vector<int> visited(N,-1);    // -1 : Nothing(not visited), 0 : Red , 1 : Green
    for(int i=0;i<N;i++)
    {
        if(visited[i] == -1)
            cout << (boolalpha) << isBipartite_(i,visited) << "\n";
    }
}
